...
architecture rtl of fir_filter_4 is
  ...
  type t_add_st0 is array (0 to 1) of signed(31 downto 0);

  signal r_coeff   : t_coeff;
  signal p_data    : t_data_pipe;
  signal r_mult    : t_mult;
  signal r_add_st0 : t_add_st0;
  signal r_add_st1 : signed(31 downto 0);
...
begin
  ...
  begin
    if(i_rstb = '0') then
      ...
      o_data     <= std_logic_vector(resize(p_data(0)*r_coeff(0) + p_data(1)*r_coeff(1) + p_data(2)*r_coeff(2) + p_data(3)*r_coeff(3), 32));
    end if;
  end process p_input;
...
--        r_add_st0(k) <= resize(r_mult(2*k), 32) + resize(r_mult(2*k+1), 32);
--      end loop;
--    end if;
...
--      r_add_st1 <= (others => '0');
--    elsif(rising_edge(i_clk)) then
--      r_add_st1 <= resize(r_add_st0(0), 32) + resize(r_add_st0(1), 32);
--    end if;
...
--      o_data <= (others => '0');
--    elsif(rising_edge(i_clk)) then
--      o_data <= std_logic_vector(r_add_st1(31 downto 0));
--    end if;
